        ...
        if (text == "accel" || text == "mag" || text == "gyro") {
            ...
            if (text == "accel") {
                ...
                if (_autopilot->paramExists(PX4AutopilotPlugin::defaultCompId, "CAL_MAG_SIDES")) {
                    ...
                    sides = _autopilot->getParamFact(PX4AutopilotPlugin::defaultCompId, "CAL_MAG_SIDES")->rawValue().toFloat();
                } else {
...
void SensorsComponentController::_refreshParams(void)
{
    ...
    foreach (const QString &paramName, fastRefreshList) {
        _autopilot->refreshParameter(PX4AutopilotPlugin::defaultCompId, paramName);
    }

    // Now ask for all to refresh
    _autopilot->refreshParametersPrefix(PX4AutopilotPlugin::defaultCompId, "CAL_");
    _autopilot->refreshParametersPrefix(PX4AutopilotPlugin::defaultCompId, "SENS_");
}
